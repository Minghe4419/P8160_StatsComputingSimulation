...
# 4.
comparison1 <- as.data.frame(rbind(c("bisection", res_f1_bisec), 
                                  c("golden search", res_f1_gs),
                                  c("newton method", res_f1_nt)))
comparison1 <- comparison1 %>% 
  rename(name = "V1", result = "V2", iteration = "V3")
comparison1

comparison2 <- as.data.frame(rbind(c("bisection root 1", res_f2_bisec1),
                                   c("bisection root 2", res_f2_bisec2),
                                   c("golden search root 1", res_f2_gs1),
                                   c("golden search root 2", res_f2_gs2),
                                   c("newton method root 1", res_f2_nt1),
                                   c("newton method root 2", res_f2_nt2)))
comparison2 <- comparison2 %>% 
  rename(name = "V1", result = "V2", iteration = "V3")
comparison2
```

1. **function A: **  $f'_1(x) = \frac{2x}{1+x^2} + 2x$, $f''_1(x) = \frac{2(x^4 +x^2 +2)}{(1+x^2)^2}$

  **function B: ** $f'_2(x) = 4x^3 - 12x + 4$, $f''_2(x) = 12x^2 - 12$

2. To find whether functions are unimodal, we need to find root where first prime of function is 0 and second prime of function is positive.
...
3. The comparison of results for function A and B are presented above in tables.

By question 2, we know that root of function B are: `r critical_points_f2`, and their second derivatives are `r f2_second(critical_points_f2)`. Therefore, we only have 2 local minimum.
