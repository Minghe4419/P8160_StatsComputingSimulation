...
```
First, we notice `nox` has strong effects on highest and cheapest group of housing, but its confidence interval on medium price housing always includes 0, indicating this could be a insignificant/ non-influencial predictor at given prices. Then for other predictors: `rm` and `chas` have stronger effect for expensive homes; lstat has stronger effect for cheaper homes; `tax`'s effect as a variable is negligable.
